...
import type { llmOptions, llmProvider, llmStreamBreaker, llmStreamHandler } from './llm/llm' // Adjust path if necessary
...
const mockLlmProviders: llmProvider[] = [
  {
    ...
    localApiKey: '',
    models: (_s: string): Promise<string[]> => {
      throw new Error('Function not implemented.')
    },
    start: (
      _credential: string,
      _session: Session,
      _handleDelta: llmStreamHandler,
      _opts: llmOptions,
...
describe('ConfigDialog', () => {
  ...
  it('should save Polly credential changes to localStorage', () => {
    ...
    const pollyInput = screen.getByLabelText(
  ...
  it('should save Polly Voice ID (JP) changes to localStorage', () => {
    render(<ConfigDialog open={true} llmProviders={mockLlmProviders} onClose={mockOnClose} />)
