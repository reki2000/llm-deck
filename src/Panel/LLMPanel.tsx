    ...
    <Stack spacing={1} width="100%">
      <Stack direction="row" spacing={1} display="flex" alignItems="center">
        ...
        <Stack  direction="row" display="flex" alignItems="center">
          ...
          <MinWidthButton onClick={() => setMarkdown((v) => !v)}>{markdown ? <FormatClearIcon/> : <FormatSizeIcon/>}</MinWidthButton>
          <MinWidthButton onClick={close}><CloseIcon/></MinWidthButton>
        </Stack>
      </Stack>
      ...
      {working ? "[working...]"  
        : (
        <Stack direction="row" spacing={1} display="flex" alignItems="center">
          <TextField label="Reply" fullWidth
          ...
          />
          <Button  sx={{minWidth: 'auto'}} onClick={() => handleReply(response, reply)}><SendIcon/></Button>
        </Stack>
        )}
    </Stack>
  )
